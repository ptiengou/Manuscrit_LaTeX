...
\begin{titlepage}
    ...
    {\Large SORBONNE UNIVERSITE\par}
    % \vspace{0.1cm}
    {\Large UMR METIS 7619\par}
    % \vspace{0.1cm}
    {\Large Laboratoire de Météorologie Dynamique\par}
    \vspace{0.1cm}
    {\Large Ecole Doctorale 398 - \large Géosciences Ressources Naturelles et Environnement\par}
    \vspace{1cm}

    {\Large Thèse pour obtenir le grade de Docteur de Sorbonne Université}
    \vspace{0.1cm}

    {\Large Spécialité : Hydrologie}
    \vspace{0.5cm}

    {\hrule height 1pt} % Ligne fine
    \vspace{0.5cm}
    ...
    {\Large\bfseries Pierre Tiengou\par}
    \vspace{1cm}

    {\large Devant un jury composé de\par}
    \vspace{0.5cm}

    \hspace*{-0.7cm}
    \resizebox{1.1\textwidth}{!}{
    \begin{tabular}{@{}ll@{}}
    {\large\bfseries Fabienne LOHOU},       \normalsize Professeure des universités, LAERO - U. Toulouse III        & \large Rapportrice \\
    {\large\bfseries Isabelle BRAUD},       \normalsize Directrice de Recherche, INRAE RiverLy                      & \large Rapportrice \\
    {\large\bfseries Jean-Louis DUFRESNE},  \normalsize Directeur de Recherche, LMD - Sorbonne U.                   & \large Examinateur \\
    {\large\bfseries Romain ROEHRIG},       \normalsize Ingénieur des Ponts, des Eaux et des Forêts, CNRM                                   & \large Examinateur \\
    {\large\bfseries Martin BEST},          \normalsize Science Fellow, UK Met Office                              & \large Examinateur \\
    {\large\bfseries Aaron BOONE},          \normalsize Directeur de Recherche, CNRM                                & \large Examinateur \\
    {\large\bfseries Agnès DUCHARNE},       \normalsize Directrice de Recherche, METIS - Sorbonne U.                & \large Co-directrice de thèse \\
    {\large\bfseries Frédérique CHERUY},    \normalsize Chargée de Recherche, LMD - Sorbonne U.                     & \large Co-directrice de thèse \\
    \end{tabular}
    }
\end{titlepage}
...
Several biases of the ICOLMDZOR LAM over the region were also identified and partly corrected, and possible improvements are presented for upcoming regional climate modelling studies.
...
Cette thèse étudie les effets de l'irrigation sur le cycle de l'eau, le climat régional et la couche limite atmosphérique de la péninsule Ibérique, à l'aide du modèle à aire limitée (LAM) ICOLMDZOR, à 25 km de résolution. Ce nouveau modèle climatique régional est dérivé du modèle climatique global de l'Institut Pierre-Simon Laplace (IPSL-CM). Il associe le modèle de surface continentale ORCHIDEE, doté d'un nouveau schéma de routage des rivières au modèle atmosphérique ICOLMDZ.
...
Les processus atmosphériques en jeu sont analysés plus en détail en comparant le LAM ICOLMDZOR à des données d'observation issues de la campagne LIAISE (Land Surface Interactions with the Atmosphere over the Iberian Semi-Arid Environment) effectuée dans la vallée de l'Ebre en Juillet 2021.
...
Par rapport à une simulation Meso-NH à 2 km de résolution, les variables de surface dans la maille ICOLMDZOR du site irrigué correspondent à la moyenne des mailles Meso-NH qu'elle contient, suggérant que la modélisation de l'irrigation et des flux de surface dans ORCHIDEE est suffisante pour représenter les impacts moyennés sur la maille, à la surface. 
Cependant, bien que le refroidissement et l'humidification induits par l'irrigation se propagent verticalement dans la CLA, ICOLMDZOR ne représente pas aussi bien la CLA que les variables de surface. Cela est vraisemblablement attribuable pour partie à l'influence des hétérogénéités sous-maille, non seulement dans les flux de surface, mais aussi dans la vitesse et la direction du vent, ainsi qu'à des termes d'advection qui ne reproduisent pas parfaitement les conditions météorologiques de la campagne.
...
\chapter{Influence of lateral boundary conditions on the consistency of the LAM}
\label{chap:forcing}
\begin{minipage}[t]{0.9\textwidth}
    \minitoc
\end{minipage}
...
\chapter{Diurnal impacts of irrigation and surface heterogeneities on the ABL}
\label{chap:liaise}
\begin{minipage}[t]{0.9\textwidth}
    \minitoc
\end{minipage}
