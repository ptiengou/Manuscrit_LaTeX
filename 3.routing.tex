...
This chapter presents the preliminary work to evaluate and calibrate the new \native river routing scheme before running coupled simulations with ICOLMDZOR. As a reminder, the pre-existing version of the routing scheme, \std, cannot be used with ICOLMDZOR because it imposes constraints on the ORCHIDEE and routing grids that are incompatible with the icosahedral grid. 
...
\noindent Two objectives were initially identified in this study:
\begin{itemize}
...
Topographical data from the two DEMs presented in Chapter \ref{chap:methods} was used, either from the 0.5°-resolution DEM historically associated with the \std routing or from the MERIT DEM at 1-arcminute resolution.
...
\begin{figure}[htbp]
    ...
    \caption{River discharge stations selected for the routing scheme evaluation and parameter tuning.}
...
\begin{table}[hbtp]
    ...
    \resizebox{\textwidth}{!}{%
        ...
        \hline
        \textbf{Station} & \textbf{River} & \textbf{Area (km$^2$)} & \textbf{Distance error (km)} & \textbf{Area error} \\
        \hline
        1 (Zaragoza)          & Ebro     & 40434 & 0.505 & 0.0047 \\
        2 (Albarreal de Tajo) & Douro    & 27159 & 0.978 & 0.0127 \\
        3 (San Miguel del Pino)& Tagus    & 36570 & 0.660 & 0.0014 \\
        4 (Badajoz)           & Guadiana & 48530 & 0.895 & 0.0046 \\
        \hline
    \end{tabular}
    }
    \caption{Characteristics (name, river, upstream catchment area, and postiionning errors) of river discharge stations used for the routing scheme evaluation and parameter tuning.}
...
25            & 3             & 0.24            \\ \hline
\end{tabular}
\caption{Routing time constant used for the consistency analysis (day km$^{-1}$).}
...
\begin{figure}[htbp]
    ...
    \caption{Time series and mean seasonal cycles (2003-2012) of reservoir volumes on average over the non-coastal grid cells of the Iberian Peninsula domain.}
...
Regarding the river reservoir (Fig. \ref{fig:reservoir_cont_time_series}f), \std simulates much lower volumes in \irr than in \noirr (-75\% in winter and complete depletion in summer). This decrease is also seen with \native, and the differences between the two routing versions almost disappear in the  \irr simulations, meaning that all the additional river water available in \std is used by the irrigation parametrization. 
...
\begin{figure}[ht!]
    ...
    \end{subfigure}
    \caption{Simulated irrigation in \std and \native. Annual mean (2003-2012) for both simulations (a,b), absolute and relative difference (c-d). Average time series (e) and seasonal cycle (f) over the Iberian Peninsula.}
...
\noindent The parameter tuning pursued three objectives:
\begin{itemize}
...
\item The values used over France with the \textit{subgrid\_HTU} routing and the 1-arcminute input DEM \citep{rinchiuso_improving_2022, huang_multi-objective_2024}.
\end{itemize}
...
It can be noted that the performance of TCST1 for river discharge is good, but that it holds almost no water in the groundwater and overland reservoirs, suggesting that this previous calibration with the 1-arcminute MERIT DEM which focused only on matching discharge observations \citep{kilic_evaluation_2023} neglected some aspects of the routing scheme and effectively calibrated only TCST\_STREAM.

The sensitivity of simulated river discharge to irrigation was then explored using the new set of parameters TCST4 for the routing scheme, as well as the influence of the atmospheric forcing.
