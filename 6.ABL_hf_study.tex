...
In all the following figures, 15 July will be shown on the left and 20 July on the right. The surface conditions at La Cendrosa on these days are first compared to understand their differences ( see supplementary Fig. \ref{fig:iop_days_TS_surfvars_elsplans} and \ref{fig:iop_days_TS_energy_elsplans} for the Els Plans site), before analysing vertical profiles over both sites, and assessing the influence of sub-grid heterogeneities.
...
The downwelling longwave radiation flux (LW down, Fig. \ref{fig:iop_days_TS_energy}g-h) can be considered to summarise several differences between 15 July and 20 July. 
...
This analysis remains very simplified and assumes that some of the characteristics of near-surface variables can be extended to the rest of the ABL. This assumption is confirmed by the comparison of vertical profiles in the models and radio-soundings at 12UTC presented in the next section, providing further insight into the behaviour of the atmosphere on these two days.
...
\begin{figure}[hbtp]
    ...
    \caption{Vertical profiles of potential temperature and specific humidity at 12UTC at La Cendrosa (a-d) and Els Plans (e-h), on 15 July and 20 July. Additional orange lines show the mean profile for the minimum (dotted) and maximum (dashed) sensible heat fux sub-samples (first and last two bins on Fig. \ref{fig:sens_bins}).}
...
Among the possible causes of such discrepancies in vertical structure are the ABL parametrizations of LMDZ, the influence of sub-grid heterogeneities, and variables which are mostly determined by the dynamics. 
Vertical profiles of wind are not displayed here for conciseness, but show almost no differences between the three ICOLMDZOR simulations (see supplementary Fig. \ref{fig:profiles_winds_sensbins}). The the direction simulated by ICOLMDZOR is mostly correct but that wind speed might be too low in most of the ABL on both sites, although it is really similar to \mesomean and \mesoexact. 
...
At Els Plans potential temperature is correct in the mixed layer, but there is an even larger dry bias of 2.5 g \perkg (Fig. \ref{fig:profiles_theta_ovap_sensbins}g, h). It must be noted that this is not because ICOLMDZOR simulates a lower specific humidity (in \noirr it is similar on both sites) but rather because the observed values are higher in the rain-fed site than in the irrigated site, suggesting a strong impact of non-local processes.
The activation of irrigation at La Cendrosa enables large improvements of the warm and dry biases in \irrboost. Potential temperature is decreased by at least 1K (more than half of the bias), and all the mixed layer benefits from the surface moistening, allowing specific humidity to match the radio-sounding profile. 
...
The main takeaway from this analysis of vertical profiles is that the impact of irrigation in \irrboost is not limited to surface turbulent fluxes or 2-metre temperature and specific humidity, but also impacts the atmospheric boundary layer.

This allows irrigation to reduce the warm bias in the ABL on both days, reducing potential temperature by 1K, and leads to an increase in specific humidity of 1-1.5 g \perkg over La Cendrosa.
...
To further investigate the mixing in the ABL and understand what controls its vertical extension on both days, the influence of sub-grid heterogeneities was explored in more details.
%todoFC:tu viens de dire que il y un gros impact de la dynamique. Je ne tournerais pas la phrase de cette façon.
...
%todoFC:le premier niveau des meso-NH est a quelle altitude? Tu ne peux pas prolonger le profils LMDZ jusque à la surface au moins avec t2m?
...
At Els Plans (Fig. \ref{fig:sens_bins}c-d), the distribution is more symmetric, although the presence of some irrigated grid cells within the considered ensemble lead to a longer tail of data points with weak sensible heat flux.
...
At La Cendrosa on 15 July, it shows that the values of potential temperature and specific humidity are very similar for the two sub-samples and the mean value, suggesting that mixing is efficient.
The mean boundary layer is very similar to the low surface sensible heat flux sub-sample which contains about 50\% of grid cells (Fig. \ref{fig:profiles_theta_ovap_sensbins}a-b). In particular, the inversion is at the same height, suggesting they are the major driver of ABL height in the area. The ABL is higher for the high sensible heat flux sub-sample, but still much lower than in \noirr and \irr. This can suggest that the ABL in dry conditions is too high in ICOLMDZOR, or that the extent of the ABL over these dry grid cells is limited because they do not cover a sufficient surface to generate their own mixed layer.

At Els Plans (Fig. \ref{fig:profiles_theta_ovap_sensbins}e-f), there are large differences in potential temperature (more than 2K) and specific humidity (1 g \perkg) between the two sub-samples.
...
The mixing induced by strong winds and the upward motions associated to the convergence of air (see vertical winds in supplementary Fig. \ref{fig:iop_days_vertwinds}) are most likely the cause for the very high ABL on 20 July (higher than on 15 July despite lower sensible heat fluxes in observations and Meso-NH). 
...
In Meso-NH, above the front, higher altitude south-easterly winds seem to partly advect that well developed mixed layer into the irrigated zone, above the internal boundary layer, contributing to the more complex vertical profiles at La Cendrosa.
Some further investigation could help determine if the location of the front is mainly dictated by the difference in roughness or by pressure gradients between the two zones, but it seems clear that sub-grid heterogeneities of surface sensible heat flux are not a first order driver of ABL development.
...
%todoFC:C est vraiment tres negatif. est ce que ces valeurs d obs. sont discutées dans la litérature sur LIAISE?.  Tu pourrais parler aussi des incertitudes sur les obs.  en particulier de la non fermeture du bilan d energie et les corrections apportée aux flux qui se base sur la conservation du rapport de bowen. On a regardé ca avec Alice. Tu ne vas pas entrer dans les détails mais il faudrait quand meme le mentionner.   Ci dessous ce que l on a mis dans le papier d Alice Eddy covariance measurement of turbulent flows results almost systematically in non-closure of the energy balance of around20 %. As presented by Mauder et al. (2024), this imbalance can come from several sources such as sensor uncertainties,275sampling error, storage terms, measurement footprint on heterogeneous landscapes, etc. To reduce energy imbalance, theground storage term is estimated in Section B and the measurement footprint in Section C. The sampling error corresponds toeddies of large or small size that are not sampled (respectively due to a too short or too long sampling period). It is calculated according to Finkelstein and Sims (2001); Jomé (2024) and is shown in Fig A1. Figure A1 shows that the residual fluctuates between 1 and 100 W.m−2, and is slightly higher on the morning of August 20 (up to 185 W.m−2, equivalent to 30 % of Rn).280 During the night, the residual is negative (-22 W.m−2 on average and around 50 % of Rn). Then, it is rather common to partition the energy balance residual into contributions from H and LE. Several methods exists Mauder et al. (2024), one of the simplest ways is to assume that the Bowen ratio is preserved, and to adjust sensitive and latent flows by the same percentage (Twine et al., 2000). This correction is done here with instantaneous Bowen ratio (30 min time step)
...
At La Cendrosa, this enabled simulated irrigation to match irrigation demand, and latent heat flux to follow a complete diurnal cycle. On average over the LIAISE SOP \irrboost largely reduced the biases to observed values of turbulent fluxes, 2-metre temperature and humidity and slightly reduced the excessive wind speed. 
...
The question of whether this good performance on surface variables was also observed in the boundary layer was investigated using vertical profiles of the atmosphere. Out of the seven IOP days on which radio-soundings were conducted on both sites, two were selected (15 and 20 July), each representative of a part of the SOP with different characteristics.
