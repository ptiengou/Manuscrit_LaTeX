...
\begin{itemize}
    ...
    \item To what extent can the size of the domain limit the influence of these inconsistencies in the free zone?
    \item How much does the source and sampling frequency of the forcing file influence the consistency and performance of the LAM ?
\end{itemize}
...
\begin{figure}[htbp]
    ...
    \begin{tabular}{ccc}
        ...
        \end{subfigure}
    \end{tabular}
    \caption{Annual mean (2010-2014) of the six variables used for the analysis of the LAM structural biases in ERA5.}
...
The overestimation in ET is also confined to the north-western edge of the domain and the large overestimation in the Mediterranean is reversed to an underestimation, of smaller magnitude, for both \interd and \larged (Fig. \ref{fig:domain_size_P_ET_ERA_diff_maps}e-f).
...
\begin{figure}[htbp]
    ...
    \begin{tabular}{ccc}
        ...
        \end{subfigure} 
    \end{tabular}
    \caption{Annual mean (2010-2014) precipitation (a-c) and ET (d-f) biases compared to ERA5 for three simulations with small, intermediate and large domain sizes.}
...
\begin{figure}[ht!]
    ...
    \begin{tabular}{ccc}
        ...
        \end{subfigure}
    \end{tabular}
    \caption{Annual mean (2010-2014) cloud cover and downwelling radiative fluxes biases to ERA5 for three simulations with small, intermediate and large domain sizes.}
...
\begin{figure}[htbp]
    ...
    \end{subfigure}
    \caption{Mean seasonal cycle (2010-2019) of precipitation and evapotranspiration, on average over the Iberian Peninsula, for the LAM forced by ERA (red) and forced by ICOLMDZOR (blue), and the GPCC and GLEAM products.}
...
\begin{figure}[!ht]
    ...
    \begin{tabular}{cc}
        ...
        \end{subfigure} \\
    \end{tabular}
    \caption{Annual biases (2013) of precipitation, evapotranspiration, and downwelling radiative fluxes for the two simulations, with hourly and 6-hourly forcing data, compared to ERA.}
...
\begin{figure}[ht!]
    ...
    %precip
    \begin{subfigure}[b]{0.49\textwidth}
        \caption{Seasonal cycle of precipitation}
        \includegraphics[width=\textwidth]{images/chap4/forcing_sampling_freq/IP_seasonal_cycle_precip.png}
    \end{subfigure}
    \begin{subfigure}[b]{0.49\textwidth}
        \caption{Seasonal cycle of ET}
        \includegraphics[width=\textwidth]{images/chap4/forcing_sampling_freq/IP_seasonal_cycle_evap.png}
    \end{subfigure}
    \caption{Mean seasonal cycle (2013) of precipitation and evapotranspiration, on average over the Iberian Peninsula, for the LAM forced with hourly (blue) and 6-hourly (red) forcing data, and the GPCC and GLEAM products.}
...
As a reminder, the setup used for this analysis of the sampling frequency was limited to one simulation year (although repeated multiple times), and different conclusions might be obtained in other experiments over a longer period, or with a different forcing source that does not generate such strong inconsistencies in the transition zone.
In his internship report, \citet{conesa2022} had analysed the sensitivity of the LAM (in an idealised setup with simplified physics) and found that hourly data performed very similarly to higher sampling frequency (down to 5 mn), that errors became significant for 3-hourly data and remained largely acceptable for 6-hourly data, but that performance was further degraded for 12-hourly data.
