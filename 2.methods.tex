...
\begin{figure}[ht]
    ...
    \caption{Example of the structure of the LAM domain with NBP=27 \citep[from][]{raillard_leveraging_2024}}
    \label{fig:LAM_domain}
\end{figure}

The principles of nudging are well known in climate modelling and NWP. It consists in adding a forcing term in the equation that governs the evolution of a given prognostic variable V in the model, to drive it towards a reference value $V_{ref}$ over a given time scale $\tau$.
...
%are read at each time step of the dynamics, and 
