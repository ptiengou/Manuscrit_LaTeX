...
Icosahedral dynamical cores already existed in some models, such as ICON \citep{zangl_icon_2015,giorgetta_icon-_2018,prill_icon_2022}, and was introduced in the IPSL-CM with the main objective of standardizing grid cells for global simulations and unlocking a high level of parallelization. In particular, it avoids the discrepancies of a regular longitude-latitude grid that appear at the poles, opening new prospects for modelling projects in the Arctic \citep{raillard_leveraging_2024} and Antarctic regions \citep{wiener_extensive_2025}.
...
In addition to meteorological variables, the LSM requires other input data. Each grid cell must be assigned a soil texture which determines various hydrological and thermal soil parameters. 
...
Although they share the same soil texture within a grid cell, these soil tiles feature an independent water budget because the PFTs influence infiltration and evapotranspiration.
...
%todo:préciser en couplé si on prend l'humidité au premier niveau ou interpolée à 2m (même question pour vent, 10m ou premier niveau?...) On prend tout au premier niveau

$C_{drag}$ is computed for the whole grid cell and depends on two roughness lengths: $z_{0m}$, below which wind speed is assumed to be zero, and $z_{0h}$, below which air temperature is assumed to be equal to the surface soil temperature. These roughness lengths are computed for each PFT then aggregated to obtain a unique value for the grid cell. This modelling choice is referred to as \textit{parameter aggregation}, as opposed to \textit{flux aggregation} methods, used in some land surface models, which compute turbulent fluxes for each PFT before computing an average value to represent the grid cell.
The default parametrization of roughness lengths in ORCHIDEE 2.2 is based on \citet{su_evaluation_2001} and calculates them both dynamically, using empirical formulations. However, some of the obtained values are not always consistent (particularly for $z_{0h}$), and another parametrization was introduced in ORCHIDEE, which will serve as default for CMIP7.
With this simpler approach, the two roughness lengths are constant over time, and depend on two prescribed parameters for each PFT:  %todo:check car doute sur le fait que la canopy height soit constante...
\begin{itemize}
...
The heat flux in the ground is computed using the semi-implicit coupling scheme mentioned earlier, at the same time as the turbulent diffusion is the atmospheric column \citep{polcher_proposal_1998}
%todo:refs cf plus haut idem mettre toutes les ref surtout pour le flux dans le sol, cela vient de la these d hourdin . 
...
Le bilan d energie a la surface est evalué implicitement en utilisant les formulation des sensibilites des flux turbulents et LW up aux variations aux variations de température de surface d'une part et deux systemes tridiagonaux dans l atmosphere et dans le sol pour resoudre implicitement les equations de diffusion.  Tu peux aller voir mon cours de cet année sur le couplage eventuellement
...
In this version, the routing grid is exactly the same as the DEM grid, meaning that the HTUs are exactly the DEM grid cells. This scheme relies on a robust interpolation %todo:ref Kritsikis, E., Aechtner, M., Meurdesoif, Y., and Dubos, T.: Conservative interpolation between general spherical meshes, Geosci. Model Dev., 10, 425–431, https://doi.org/10.5194/gmd-10-425-2017, 2017
...
\end{figure}
%todo:refaire figure sans corrections orthographiques ppt
...
The only variable that is output directly on the routing grid is the river discharge. This enables a more relevant comparison to discharge observation stations than using an average discharge value over the ORCHIDEE grid cell. 
...
\end{figure}  %todo:refaire screenshot sans corrections orthographiques ppt